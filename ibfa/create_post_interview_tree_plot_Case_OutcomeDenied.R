...
# output new_data
file_name <- "output/post_interview_wo_issue/output/post_interview/full/treatment_post_interview_full_w_tree_output_Case_OutcomeDenied"
...
file_name <- "output/post_interview_wo_issue/output/post_interview/full/treatment_post_interview_full_tree_hierarchy_Case_OutcomeDenied"
current_date <- Sys.Date()
current_date <- str_replace_all(current_date, pattern = "-", replacement = "")
file_name <- str_c(file_name, "_", current_date, ".csv")
file_name
...
tree_output_w_hierarchy <- read_csv("output/post_interview_wo_issue/output/post_interview/full/treatment_post_interview_full_tree_hierarchy_Case_OutcomeDenied_20180829.csv")
...
# reid's confidence intervals for IBFA are very slightly off what i get (based on his handling of NA values)
# note his conf. int. for non-IBFA (control) are the same as what i get
...
# inspect original conf. intervals
tree_output_w_hierarchy %>% data.frame()
tree_output_w_hierarchy %>% select(terminal_node, outcome_p_weighted, upper_limit_outcome_p_weighted, lower_limit_outcome_p_weighted)

# load edited conf. int
edited_denial_rate_conf_int <- read_csv("post_interview_ibfa_denial_rate_confidence_intervals_edited_from_reid.csv")
...
# bind edited conf_int with tree_output_w_hierarchy for use in tree_plot
...
# load post_interview_full_output_summary_stats (created with get_output_summary_stats_for_all_post_interview_nodes.R)
# this significance, unreliable, and suppressed flags to add to tree
post_interview_output_summary_stats <- read_csv("output/post_interview_wo_issue/output/post_interview/full/post_interview_full_output_summary_stats.csv")
glimpse(post_interview_output_summary_stats)

# note that node 12 was not validated train/test, but is significant overall, and it is only flagged unreliable for failing validation
# it's n = 32 and coeff of var = .313, so reid's table does not flag this
# this means that reid's unreliable/suppresed flags (which don't account for failure to validate train/test) need to be changed
# node 5 also failed to validate train/test, but was already flagged unreliable due to n = 10 and cv = .39, so no change to reid's table needed
post_interview_output_summary_stats %>% select(node, treatment_p_weighted, treatment_p_weighted_lower_bound, treatment_p_weighted_upper_bound, 
                                               treatment_p_diff_overall, treatment_p_diff_overall_lower_bound, treatment_p_diff_overall_upper_bound,
                                               node_train_test_validated_significant, treatment_p_diff_overall_significant, 
                                               suppressed_treatment_p, unreliable_treatment_p, treatment_n, treatment_p_coeff_of_variation) %>%
                                                data.frame()

post_interview_output_summary_stats %>% select(node, treatment_p_weighted_fraud, treatment_p_weighted_fraud_lower_bound, treatment_p_weighted_fraud_upper_bound, 
                                               treatment_p_diff_fraud_overall, treatment_p_diff_overall_fraud_lower_bound, treatment_p_diff_overall_fraud_upper_bound,
                                               node_train_test_fraud_validated_significant, treatment_p_diff_overall_fraud_significant, suppressed_treatment_p_fraud,
...
tree_output_w_hierarchy <- post_interview_output_summary_stats %>% 
        select(node, unreliable_treatment_share, suppressed_treatment_share,
               treatment_p_diff_overall_significant, suppressed_treatment_p, unreliable_treatment_p, 
               treatment_p_weighted_fraud, # treatment_p_weighted_fraud_upper/lower_bound_edited are already added separately below
...
# add edited conf. int for fraud rate
# load edited denial rate conf. int
edited_fraud_rate_conf_int <- read_csv("post_interview_ibfa_fraud_rate_confidence_intervals_edited_from_reid.csv")
...
# add edited significance/unreliable/suppressed info for denial rate
edited_denial_rate_significance_unreliable_suppressed_flags <- read_csv("post_interview_ibfa_denial_rate_significance_unreliable_suppressed_flags_edited_from_reid.csv")
edited_denial_rate_significance_unreliable_suppressed_flags 

# join with tree_output_w_hierarchy
glimpse(tree_output_w_hierarchy)
tree_output_w_hierarchy <- tree_output_w_hierarchy %>% left_join(., edited_denial_rate_significance_unreliable_suppressed_flags, by = "terminal_node")
glimpse(tree_output_w_hierarchy)
...
edited_rates_and_intervals_for_nodes_2_and_3 <- read_csv("post_interview_nodes_2_and_3_all_rates_and_intervals_edited_from_reid.csv")
...
tree_output_w_hierarchy %>% filter(terminal_node %in% c("3", "2")) %>% 
        select(treatment_p_weighted_fraud, lower_limit_outcome_p_weighted_fraud_edited, upper_limit_outcome_p_weighted_fraud_edited,
               outcome_p_weighted, upper_limit_outcome_p_weighted_edited, lower_limit_outcome_p_weighted_edited) %>% data.frame()
...
tree_output_w_hierarchy <- tree_output_w_hierarchy %>% 
        ...
        mutate(outcome_p_weighted_output = case_when(ibfa_denial_rate_significant_edited == 0 ~ as_percent(outcome_p_weighted, 1),
       ...
       mutate(outcome_p_weighted_fraud_output = case_when(suppressed_treatment_p_fraud == 1 ~ "-", 
                                                    TRUE ~ treatment_p_weighted_fraud_output)) %>%
...
# save plot
tree_plot_title <- str_c("post_interview_tree_plot_", "SOF_Finding_SPFF")
tree_plot_title

filename <- str_c("output/post_interview_wo_issue/output/post_interview/train/", tree_plot_title, ".pdf")
filename <- str_c("output/post_interview_wo_issue/output/post_interview/train/", tree_plot_title, ".png")
filename
...
# save plot as png
ggsave(filename = "post_interview_plot_w_legend_png.png", plot = plot_w_legend, dpi = 300)
ggsave(filename = "plot_w_legend_pdf.pdf", plot = plot_w_legend, dpi = 300)


# load png into magick
plot_w_legend_image <- image_read(path = "post_interview_plot_w_legend_png.png")
plot_w_legend_image
...
# + 100px from topmost
# legend <- image_crop(image = plot_w_legend_image, geometry = "700x550+900+550")
legend <- image_crop(image = plot_w_legend_image, geometry = geometry_area(width = 700, height = 550, x = 900, y = 550))
legend 

# save legend
image_write(image = legend, path = "post_interview_legend.png", format = 'png', density = '300x300')
image_write(image = legend, path = "post_interview_legend.pdf", format = 'pdf', density = '300x300')

# read legend
legend_bitmap <- pdf_render_page(pdf = "post_interview_legend.pdf", page = 1, dpi = 300)
...
ggsave(filename = "post_interview_plot_w_footnote_png.png", plot = plot_w_footnote, dpi = 300,
       width = 15)
ggsave(filename = "post_interview_plot_w_footnote_pdf.pdf", plot = plot_w_footnote, dpi = 300,
       width = 15)

# load png into magick
plot_w_footnote_image <- image_read(path = "post_interview_plot_w_footnote_png.png")
...
# image_crop(image, "100x150+50+100"): crop out width:100px and height:150px starting +50px from the leftmost, 
# + 50px from topmost
footnote <- image_crop(image = plot_w_footnote_image, "2800x400+100+930")
footnote 

# save footnote
image_write(image = footnote, path = "post_interview_footnote.png", format = 'png', density = '300x300')
image_write(image = footnote, path = "post_interview_footnote.pdf", format = 'pdf', density = '300x300')

# read legend
footnote_bitmap <- pdf_render_page(pdf = "post_interview_footnote.pdf", page = 1, dpi = 300)
...
# read tree_plot pdf
tree_plot_pdf_bitmap <- pdf_render_page(pdf = "output/post_interview_wo_issue/output/post_interview/train/post_interview_tree_plot_SOF_Finding_SPFF.pdf",
                                        page = 1, dpi = 300)
# tree_plot_pdf_bitmap <- pdf_render_page(pdf = "output/post_interview_wo_issue/output/post_interview/train/post_interview_tree_plot_Case_OutcomeDenied.pdf",
#                                         page = 1, dpi = 300)
...
tree_plot_w_legend_image <- image_composite(image = image_border(image = tree_plot_pdf_image, color = "white", 
                                                                 geometry = "1000x1000"), 
                                            composite_image = image_resize(image = legend, geometry = "1000x1000"), offset = "+6500+1300") 
tree_plot_w_legend_image

# sizing for plot without fraud rate
# tree_plot_w_legend_image <- image_composite(image = image_border(image = tree_plot_pdf_image, color = "white", 
#                                                                  geometry = "1000x1000"), 
#                 composite_image = image_resize(image = legend, geometry = "1000x1000"), offset = "+6500+1200") 

# overlay footnote
tree_plot_w_legend_and_footnote_image <- image_composite(image = tree_plot_w_legend_image,
        composite_image = image_resize(image = footnote, geometry = "5500x5500"), offset = "+1000+4900") %>%
        image_crop(image = ., "6700x4300+1000+1000")
tree_plot_w_legend_and_footnote_image

# sizing for plot without fraud rate
# tree_plot_w_legend_and_footnote_image <- image_composite(image = tree_plot_w_legend_image, 
#         composite_image = image_resize(image = footnote, geometry = "5500x5500"), offset = "+1000+3800") %>% 
#         image_crop(image = ., "6700x3200+1000+1000") 
...
image_write(image = tree_plot_w_legend_and_footnote_image, format = "png",
            path = "output/post_interview_wo_issue/output/post_interview/train/post_interview_tree_plot_w_legend_and_footnote_SOF_Finding_SPFF_20190313.png",
            density = "300x300")

# pdf file is way too big ~ 13 MB
# image_write(image = tree_plot_w_legend_and_footnote_image, format = "pdf", 
#             path = "output/post_interview_wo_issue/output/post_interview/train/post_interview_tree_plot_w_legend_and_footnote_Case_OutcomeDenied.pdf",
#             density = "300x300")

# image_write(image = tree_plot_w_legend_and_footnote_image, format = "png",
#             path = "output/post_interview_wo_issue/output/post_interview/train/post_interview_tree_plot_w_legend_and_footnote_Case_OutcomeDenied.png",
#             density = "300x300")
